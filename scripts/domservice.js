...
export class DomService {
  ...
  getDailyDetails(clickedCard, lat, lon) {
    searchService.fetchWeatherData(lat, lon).then((data) => {
  ...
  expandWeatherDetails(
    event,
    lat = searchService.lat,
    lon = searchService.lon
  ) {
  ...
  currentLocation() {
    document.querySelector("#location-suburb").innerText =
      "Retrieving location...";
  ...
  createListItem(location) {
    ...
    newListItem.addEventListener("click", () => {
      ...
      const targetFavorite = this.favoriteList.find(
        (fav) =>
      ...
      if (targetFavorite) {
        searchService.updateLocation(
          targetFavorite.lat,
          targetFavorite.lon,
          targetFavorite.suburb ||
            targetFavorite.town ||
            targetFavorite.municipality
        );
        ...
        searchService
          .coordsToLocation(targetFavorite.lat, targetFavorite.lon)
          .then((data) => {
            renderService.renderGeoData(data);
          });
  ...
  removeListItem(data) {
    ...
    const listItemToRemove = Array.from(favoriteListItems).find(
      (item) =>
    ...
    for (let i = localStorage.length - 1; i >= 0; i--) {
      //Loop backwards to avoid index issues while removing
  ...
  toggleFavorite() {
    searchService
      .coordsToLocation(searchService.lat, searchService.lon)
      .then((data) => {
        const favoriteItem = {
          lat: data.lat,
          lon: data.lon,
        ...
        const index = this.favoriteList.findIndex(
          (fav) => fav.lat === favoriteItem.lat && fav.lon === favoriteItem.lon
        );
        ...
        if (index > -1) {
          ...
          localStorage.setItem(
            JSON.stringify(
              favoriteItem.suburb || favoriteItem.town || favoriteItem.city
            ),
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        ...
        if (parsedValue) {
          ...
          const location =
            parsedValue.suburb || parsedValue.town || parsedValue.city;
  ...
  initialize() {
    ...
    searchService.getLocationFromString().then((data) => {
      renderService.renderGeoData(data);
    });
